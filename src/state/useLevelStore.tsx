    ...
    persist(
        (set,get)=>({
            ...
            unlockLevel:(id:number) => {
                set((state) => {
                    const updatedLevels = state.levels.map((level) => level?.id === id ? {...level,unlocked:true} : level);
                    return {levels:updatedLevels};
                })
            },
            completeLevel:(id:number,collectedCandies:number) =>{
                set((state)=>{
                    const updatedLevels = state.levels.map((level) => level.id === id ? {...level,unlocked:true,highScore:Math.max(level.highScore,collectedCandies)}:level)
                    return {levels:updatedLevels}
                })
            }
        }),{
